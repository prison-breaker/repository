...
void CCore::CreateRootSignature()
{
	...
	d3d12RootParameters[static_cast<int>(ROOT_PARAMETER_TYPE::CORE)].InitAsConstantBufferView(0);					            // CB_Core : register(b0)
...
{
	UINT bytes = (sizeof(CB_Core) + 255) & ~255;

	m_d3d12Core = DX::CreateBufferResource(m_d3d12Device.Get(), m_d3d12GraphicsCommandList.Get(), nullptr, bytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, nullptr);
	DX::ThrowIfFailed(m_d3d12Core->Map(0, nullptr, reinterpret_cast<void**>(&m_mappedCore)));
}
...
void CCore::UpdateShaderVariables()
{
	...
	m_mappedCore->m_totalTime += DT;
	m_mappedCore->m_elapsedTime = DT;
	m_d3d12GraphicsCommandList->SetGraphicsRootConstantBufferView(static_cast<UINT>(ROOT_PARAMETER_TYPE::CORE), m_d3d12Core->GetGPUVirtualAddress());
}

void CCore::ReleaseShaderVariables()
{
	if (m_mappedCore != nullptr)
	{
		m_d3d12Core->Unmap(0, nullptr);
	}
}
